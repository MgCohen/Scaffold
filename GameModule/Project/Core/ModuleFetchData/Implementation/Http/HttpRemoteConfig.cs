...
namespace GameModule.ModuleFetchData.Http
{
    ...
    public class HttpRemoteConfig : IRemoteConfig
    {
        ...
        private async Task FetchIfNeeded(IExecutionContext context)
        {
            ...
            _logger.LogInformation("[HttpRemoteConfig] Initializing fetch from {Url}", _configUrl);
            ...
            try
            {
                ...
                if (data != null)
                {
                    _logger.LogInformation("[HttpRemoteConfig] Remote fetch successful. Received {Count} keys: {Keys}", data.Count, string.Join(", ", data.Keys));
            ...
            {
                _logger.LogWarning(ex, "[HttpRemoteConfig] Remote fetch failed. Using local fallback.");
            }

            Dictionary<string, string> localData = _localConfigProvider.FetchLocalConfigs();
            _logger.LogInformation("[HttpRemoteConfig] Local config provider found {Count} keys: {Keys}", localData.Count, string.Join(", ", localData.Keys));

            if (remoteData == null)
            {
                _logger.LogInformation("[HttpRemoteConfig] Using local data as primary source.");
                _cache = localData;
            }
            else
            {
