...
- **Send-path serialization cost.** What to measure: ns/op and allocs for `DirectPushClient.SendSelfPushAsync` end-to-end through the LiveOps envelope (`/home/user/Scaffold/Assets/Packages/com.scaffold.directpush/Runtime/DirectPushClient.cs:18-26`). Tool: Unity.PerformanceTesting. Test location: `com.scaffold.directpush/Tests/SendPathBenchmarks.cs`. Scenario: stub `ILiveOpsService` returning `Task.FromResult(SendPushResponse.Ok())`; 10k iterations. Baseline: dominated by `JObject.FromObject` triple-pass (see CloudCode benchmark plan). Success: serves as upstream regression marker for the CloudCode envelope refactor.
