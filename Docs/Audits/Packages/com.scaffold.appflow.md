...
- **Smells visible at `IAsyncInitializable` call sites.** `AddressablesGateway.InitializeCoreAsync` re-implements its own initialization-once gate (`AddressablesGateway.cs:82-95`, `lock(initSync)`, `bool initialized`) because AppFlow's parallel scheduler offers no idempotency contract — every consumer reinvents init-guard. `RunCatalogSyncAsync` (`AddressablesGateway.cs:147-159`) catches `Exception when (exception is not OperationCanceledException)` and `LogWarning`s — defensive swallow that bypasses `IAppFlowErrorHandler` entirely. `PushSubscriptionService.InitializeAsync` calls `cancellationToken.ThrowIfCancellationRequested()` then awaits two non-cancellable Cloud Code calls (`PushSubscriptionService.cs:42-44`) — token plumbed but unused after entry. `LiveOpsService` and `ModuleResponseDispatchService` both take `ILayerResolver` then defer-resolve `IEnumerable<IResponseHandler>` (`ModuleResponseDispatchService.cs:42`) and cache the array — this is the recurring `ILayerResolver.Top` retry pattern that the package documents but doesn't formalize.
- **`AppFlowOutcome` consumers (1 site).** `SampleLoadingScreen.ObserveStartupAsync` (`Assets/Packages/com.scaffold.appflow/Samples/SampleLoadingScreen.cs:60`) `await appFlowRoot.Progress.WhenSessionCompleted()` then ignores `outcome.Succeeded` except for a `Debug.Log`. No production code branches on `Failed`/`Cancelled`. The typed outcome model from §3 of this audit currently buys nothing.
- **`ILayerResolver` consumers (4 sites).** `NavigationInjection` (`Assets/Packages/com.scaffold.navigation/Container/NavigationInjection.cs:8,27`), `LiveOpsService` (`LiveOpsService.cs:19`), `ModuleResponseDispatchService` (`ModuleResponseDispatchService.cs:11`), and the Navigation tests' `DeferredLayerResolver` shim (`NavigationInstallerAndInjectionTests.cs:137-141` — `Top => top ?? throw new InvalidOperationException("ILayerResolver.Top is not bound yet.")`). The shim test's existence is the smell: every consumer needs a deferred-`Top` story; AppFlow ships none.
- **`ILayerPublisher` consumers: zero outside samples.** Only `SampleAssetsLayer.PrepareAsync` publishes (per README §4.1). The "best idea here" feature has no in-repo production user.
- **AppFlow → SceneFlow → Navigation chain.** Does not exist as a chain anywhere in the repo. The only inter-package coupling is `Scaffold.AppFlow.Samples.asmdef` referencing `Scaffold.SceneFlow` so `SampleLoadingScreen.cs:3` can use `LoadingView`. Navigation references AppFlow only for `ILayerResolver` injection (`NavigationInjection.cs`); `INavigation.Open` does not flow through any `IScopeLayer`. SceneFlow has no AppFlow dependency at all — `SceneFlowService` is constructed standalone and never participates in a layer push. The "AppFlow drives SceneFlow drives Navigation" story is aspirational.

## 10. Alternatives & prior art

- **MAUI Shell** — XAML-declared `Routes` with hierarchical `ShellRoute` URI grammar and a typed back-stack. https://learn.microsoft.com/dotnet/maui/fundamentals/shell/. **Steal pattern**: typed stage keys + URI-shaped flow declaration; the Shell `Routing.RegisterRoute` model maps cleanly onto the proposed `IAppFlowStage<TContext>` of §5.1. Don't adopt the XAML.
- **Stateless** (dotnet-state-machine/stateless) — fluent C# state machine with typed `TState`/`TTrigger` enums, guards, entry/exit actions, and parameterized triggers. https://github.com/dotnet-state-machine/stateless. **Adopt**: this is what `IAppFlow<TContext>` should be. Each stage is a state, transitions are triggers, `OnEntry` runs the layer push. Drop-in NuGet, MIT, Unity-compatible.
- **Cysharp UniTask** — zero-alloc `UniTask`/`UniTaskAsyncEnumerable`, sequential `.SequentialAsync()`/`UniTask.WhenAll`, and `IUniTaskAsyncEnumerable<T>` for stage streams. https://github.com/Cysharp/UniTask. **Adopt**: replaces `Task.WhenAll(fresh.Select(p => p.InitializeAsync(ct)).ToArray())` (`ParallelScheduler.cs`) and `Task.Delay(200, ct)` (`SampleAssetsLayer.cs:24`) — closure + array allocs gone. Already a Unity de-facto.
- **VContainer `IAsyncStartable` / `IInitializable`** — VContainer ships its own initialization protocol that the project's `IAsyncInitializable` deliberately diverges from. https://vcontainer.hadashikick.jp/integrations/entrypoint. **Wrap**: rather than ship a parallel contract, derive `IAsyncInitializable` from `IAsyncStartable` (or document the divergence one paragraph in the README). Six current implementers (see §9) all re-implement `InitializeAsync(CancellationToken)` that VContainer already understands.
- **Microsoft.Extensions.Hosting `IHostedService`** — `StartAsync(ct)`/`StopAsync(ct)` with built-in timeout, graceful shutdown, and host-level lifetime events. https://learn.microsoft.com/dotnet/core/extensions/generic-host. **Steal pattern**: copy the `HostOptions.StartupTimeout` / `ShutdownTimeout` story for the missing global startup deadline (§4.18). The contract shape is already congruent with `IAsyncInitializable`.

## 11. Benchmark plan

- **Stage push/pop allocation.** *What:* per-call alloc bytes for `AppFlowHost.PushAsync` + `PopAsync` round-trip. *Tool:* `Unity.PerformanceTesting` + `GC.GetAllocatedBytesForCurrentThread`. *Test location:* `Assets/Packages/com.scaffold.appflow/Tests/PlayMode/AppFlowHostBenchmarks.cs` (new PlayMode asmdef). *Scenario:* push depth 1/4/16, layer with empty `Install` and no `IAsyncInitializable`. *Baseline:* expect 1–2 KB/push today (`EnumerateStackRootToParent` `ToArray()` snapshot at `AppFlowHost.cs:268-275`, closure + `Task[]` from `ParallelScheduler`, `LayerEntry` boxing into `Stack`). *Success:* < 256 B/push after the over-decomposed helpers are inlined and `EnumerateStackRootToParent` walks the underlying storage.
- **`PopAsync` cancellation semantics.** *What:* assert `OperationCanceledException` propagates within ≤ 1 ms of `cts.Cancel()`; assert dispose wave still runs. *Tool:* NUnit + `CancellationTokenSource`. *Test location:* `Tests/Editor/AppFlowHostCancellationTests.cs`. *Scenario:* push 4 layers each with a 100 ms async dispose, cancel mid-pop. *Baseline:* today the token is `_ = ct;` swallowed (`AppFlowHost.cs:150`) — pop runs to completion. *Success:* token is honored between awaits per §5.3.
- **Error pipeline dedup overhead.** *What:* per-`Report` cost when the same `Exception` hits the handler N times. *Tool:* `Unity.PerformanceTesting`. *Test location:* `Tests/Editor/AppFlowErrorHandlerBenchmarks.cs`. *Scenario:* 10 000 `Report` calls with the same exception (current `Exception.Data["AppFlow.Reported"]` mutation, `AppFlowErrorHandler.cs:62-65`) vs. proposed `ConditionalWeakTable<Exception, object>` (§5.4). *Baseline:* `Exception.Data` triggers `ListDictionaryInternal` allocation on first write. *Success:* CWT path < 50 ns/op, no per-call alloc.
- **`ParallelScheduler` vs. proposed `SequentialScheduler` throughput.** *What:* wall-clock for one layer with N=1/4/16/64 `IAsyncInitializable`s, each yielding once via `Task.Yield()`. *Tool:* `Unity.PerformanceTesting`. *Test location:* `Tests/PlayMode/SchedulerBenchmarks.cs`. *Scenario:* parallel = `Task.WhenAll(...)` (`ParallelScheduler.cs`), sequential = `foreach await`. *Baseline:* parallel allocates `Task[N]` + closure per wave; sequential allocates 0 extra. *Success:* sequential within 1.2× of parallel for N ≤ 16 on the main thread (Unity's single-threaded sync context blunts parallelism anyway), with measurably lower steady-state allocation.
- **`AppFlowProgress.Changed` event flood under high-frequency sub-progress.** *What:* event count + lock-acquire count per second when `HostSetSubProgress` is called at 60 Hz with identical values. *Tool:* `Unity.PerformanceTesting` + manual instrumentation. *Test location:* `Tests/Editor/AppFlowProgressFloodTests.cs`. *Scenario:* 600 calls of `HostSetSubProgress(layer, 0.5f)` within one frame. *Baseline:* today `ApplySubProgressLocked` always returns `mutated=true` (`AppFlowProgress.cs:139-164`) — 600 events, 1 200 lock acquires (RaiseChanged re-locks per §4.9). *Success:* after epsilon-skip + single-lock fix, < 5 events and < 600 lock acquires.
