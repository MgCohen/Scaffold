...
- **Optimistic registry lookup.** What to measure: cost of `CloudCodeOptimisticHandlerRegistry.TryGetEntry((TReq,TRes))` after warm-up (`Assets/Packages/com.scaffold.cloudcode/Runtime/Optimistic/CloudCodeOptimisticHandlerRegistry.cs:66-119`). Tool: Unity.PerformanceTesting + `Stopwatch`. Test location: `com.scaffold.cloudcode/Tests/OptimisticRegistry.LookupBenchmarks.cs`. Scenario: 100 registered handlers, 1M lookups for hit + miss. Baseline: < 50 ns/lookup, zero allocs after warm-up. Success: matches baseline; no `Dictionary<TKey,TValue>` boxing on the `(Type,Type)` key (verify via heap snapshot).
- **Single-flight dedup correctness under burst (correctness, not perf).** What to measure: that `CloudCodeSingleFlightCallHandler` (`Assets/Packages/com.scaffold.cloudcode/Runtime/Handlers/CloudCodeSingleFlightCallHandler.cs:16-30`) serializes 100 concurrent calls to the *same* `(module, endpoint)` and runs them concurrently across modules. Tool: NUnit EditMode + `Task.WhenAll`. Test location: `com.scaffold.cloudcode/Tests/SingleFlightCorrectnessTests.cs`. Scenario: a fake `ICloudCodeCallHandler` that records timestamps; assert non-overlapping intervals per key, overlapping across keys. Baseline expectation: per-module serialization holds; per-`(module, endpoint)` does *not* hold today (audit §4.9). Success: explicit failing test pinning the documented behavior, then a follow-up green test once keying changes to `(module, endpoint)`.
