...
`ICloudCodeService` has **zero direct consumers** in the project outside the package itself; the only call to `cloudCodeService.CallEndpointAsync<T>(string, string, object, ct)` is `LiveOpsService.CallGameApiAsync` (`/home/user/Scaffold/Assets/Packages/com.scaffold.liveops/Runtime/LiveOpsService.cs:91`). All gameplay/feature traffic funnels through `ILiveOpsService.CallAsync<TResponse>(ModuleRequest<TResponse>)`. That is strong evidence the stringly-typed `CallEndpointAsync<T>` shape is wrong: every call site that *could* use it has chosen to wrap it instead.

- `Assets/Packages/com.scaffold.liveops/Runtime/LiveOpsService.cs:88-91` — the only production caller. Wraps the string API with a typed envelope built from `KeyOf.WireOf(request)`. Smell: confirms the audit's central claim — the typed-RPC pattern lives entirely in LiveOps, not in CloudCode.
- `Assets/Packages/com.scaffold.cloudcode/Tests/CloudCodeServiceOptimisticTests.cs:51,89,111,160` — only other call sites. Tests pass literal `"TestModule"`/`"TestEndpoint"` strings; this is what real consumers would do if they used the API directly.
- `Assets/Packages/com.scaffold.cloudcode/Container/CloudCodeInstaller.cs:46` — the registration. Container scope is fine; no consumer composition root references `CloudCodeInstaller` outside the package's own tests.
- No injection sites of `ICloudCodeService` exist in `Assets/Packages/com.scaffold.ads/`, `com.scaffold.directpush/`, `GameModule/`, or `LiveOps/`. The ads & directpush features inject `ILiveOpsService` instead.

Implication: collapsing `CloudCodeService` into `LiveOpsService` (or making the typed-request overload the public API and `CallRawAsync` `internal`) would not break any consumer. The string API has effectively zero adoption outside one wrapper.

## 10. Alternatives & prior art

- **MagicOnion (Cysharp typed RPC over gRPC).** Interface-defined typed RPCs with source-generated client/server. https://github.com/Cysharp/MagicOnion. **Steal pattern.** The `[CloudCodeRequest(module, endpoint)]` + generator approach proposed in §7 is MagicOnion's `IService<T>` shape adapted to the Cloud Code module model.
- **MemoryPack-RPC / MessagePack-RPC.** Binary, source-generated, allocation-light typed RPC. https://github.com/Cysharp/MemoryPack. **Steal pattern.** Replaces `JObject.FromObject` triple-pass with a single-pass binary serializer; the wire format is locked to Cloud Code (HTTP+JSON), but the request-builder layer can still pre-allocate via source-gen.
- **gRPC-Web.** HTTP/2-friendly typed RPC for browsers and Unity. https://grpc.io/docs/platforms/web/. **Build (don't adopt).** Cloud Code is the deployment substrate; gRPC-Web replaces too much. Keep as a reference pattern only.
- **Refit (HTTP-attribute typed clients, .NET).** `[Get("/users/{id}")]`-style typed HTTP clients. https://github.com/reactiveui/refit. **Steal pattern.** The `[CloudCodeRequest("Module", "Endpoint")]` attribute pattern is Refit-shaped; the source generator emits the typed call wrapper.
- **Unity Cloud Code SDK direct (no wrapper).** `Unity.Services.CloudCode.CloudCodeService.Instance.CallEndpointAsync<T>(string, string, Dictionary<string,object>)`. https://docs.unity.com/ugs/manual/cloud-code/manual. **Wrap.** The package's chain-of-responsibility (retry/timeout/single-flight/optimistic) is real value over the raw SDK. Keep wrapping; just change the public shape.

## 11. Benchmark plan

- **`JObject.FromObject` triple-serialization.** What to measure: ns/call and bytes-allocated for `LiveOpsService.CallGameApiAsync` (one client→envelope pass, one SDK→string pass, one server→`ToObject` pass). Tool: Unity.PerformanceTesting `[Performance]` on a synthetic `OptimisticGameApiRequest` with 5/50/500-property payloads. Test location: `com.scaffold.cloudcode/Tests/CloudCodeService.SerializationBenchmarks.cs`. Scenario: 10k iterations, GC-allocs measured via `BenchmarkDotNet`-style or Unity's `Recorder` API. Baseline: ~3x JSON write cost vs. a single `JsonSerializer.Serialize` to `string Payload`. Success: switching the envelope to `string Payload` removes ≥ 60% of bytes-allocated and ≥ 40% of CPU for the 50-property case.
- **Optimistic registry lookup.** What to measure: cost of `CloudCodeOptimisticHandlerRegistry.TryGetEntry((TReq,TRes))` after warm-up (`/home/user/Scaffold/Assets/Packages/com.scaffold.cloudcode/Runtime/Optimistic/CloudCodeOptimisticHandlerRegistry.cs:66-119`). Tool: Unity.PerformanceTesting + `Stopwatch`. Test location: `com.scaffold.cloudcode/Tests/OptimisticRegistry.LookupBenchmarks.cs`. Scenario: 100 registered handlers, 1M lookups for hit + miss. Baseline: < 50 ns/lookup, zero allocs after warm-up. Success: matches baseline; no `Dictionary<TKey,TValue>` boxing on the `(Type,Type)` key (verify via heap snapshot).
- **Single-flight dedup correctness under burst (correctness, not perf).** What to measure: that `CloudCodeSingleFlightCallHandler` (`/home/user/Scaffold/Assets/Packages/com.scaffold.cloudcode/Runtime/Handlers/CloudCodeSingleFlightCallHandler.cs:16-30`) serializes 100 concurrent calls to the *same* `(module, endpoint)` and runs them concurrently across modules. Tool: NUnit EditMode + `Task.WhenAll`. Test location: `com.scaffold.cloudcode/Tests/SingleFlightCorrectnessTests.cs`. Scenario: a fake `ICloudCodeCallHandler` that records timestamps; assert non-overlapping intervals per key, overlapping across keys. Baseline expectation: per-module serialization holds; per-`(module, endpoint)` does *not* hold today (audit §4.9). Success: explicit failing test pinning the documented behavior, then a follow-up green test once keying changes to `(module, endpoint)`.
- **Retry handler exception fidelity (correctness).** What to measure: that after `MaxAttempts` `CloudCodeRateLimitedException`s, the *last* rate-limit exception is rethrown — not the synthetic `InvalidOperationException("Cloud Code retry loop exited…")` (audit §4.10). Tool: NUnit EditMode. Test location: `com.scaffold.cloudcode/Tests/RetryHandlerExceptionTests.cs`. Scenario: a fake terminal handler that always throws `CloudCodeRateLimitedException`. Success: `Assert.ThrowsAsync<CloudCodeRateLimitedException>` — currently fails, proves the bug.
- **Timeout handler cancellation.** What to measure: that `CloudCodeTimeoutCallHandler` cancels an in-flight SDK call when the per-attempt timeout elapses, not just abandons the await. Tool: NUnit EditMode + a cooperative fake. Test location: `com.scaffold.cloudcode/Tests/TimeoutHandlerCancellationTests.cs`. Scenario: fake handler awaits a `TaskCompletionSource`; assert the wrapping CT becomes cancelled and the TCS is observed. Baseline: today the SDK call leaks past the timeout (audit §7 cancellation note).
