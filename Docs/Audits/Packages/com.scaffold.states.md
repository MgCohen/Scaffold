...
All scenarios use **Unity.PerformanceTesting** + the shared `Bench.Measure(...)` helper. See [`_benchmarking.md`](./_benchmarking.md) for the helper code, run policy (Editor + PlayMode IL2CPP), and the project-wide pass/fail thresholds. Tests live under `Tests/Performance/` (new folder) with a dedicated asmdef gated by define constraint `UNITY_INCLUDE_PERFORMANCE_TESTS`. Pair every "zero allocations" claim with a companion `[Test] Bench.NoAllocations(() => ...)` so a regression hard-fails the PR.

- **Typed dispatch path: `Store.Execute<TPayload>(reference, payload)`** — *What:* allocations + ns/op for one `Execute` call against a registered `Mutator<TState, TPayload>` with one slice. *Test:* `Tests/Performance/StoreExecuteBenchmarks.cs::Execute_SingleMutator_OneSlice`. *Scenario:* `iterationsPer: 10000`, payload is `record class` (current) and `readonly record struct` (post-§4.4). *Baseline:* current path allocates 1× boxed payload (struct case) + 1× `List<>` enumerator boxing in `MutatorRunner.RunMutatorBindingsWithoutCommit` (§4.12). *Success:* zero alloc per call after §4.12 + §7.1 fixes (companion `NoAllocations` test); ≥3× ns/op improvement on struct payload path. Run on IL2CPP — generic dispatch boxing differs from Mono.
- **`Store.EnumerateAll<TState>` reentrancy + alloc** — *What:* per-call alloc and reentrancy correctness. *Test:* `Tests/Performance/StoreEnumerateAllBenchmarks.cs` for perf, plus `Tests/Reentrancy/EnumerateAllUnderNotifyTests.cs` (regular `[Test]`, not `[Performance]`) for the correctness proof — a subscriber that calls `EnumerateAll` from inside notify must not corrupt iteration. *Scenario:* 1k slices of one `TState`, 100 enumerations per measurement, then nested enumeration from a subscriber. *Baseline:* `sliceBuffer` reuse currently produces ~0 alloc on linear path but corrupts under reentry (§4.5). *Success:* identical alloc count after §7.3 (per-type indexed bucket) and zero corruption (assertions pass) under nested call.
- **`StateEventHandler.NotifyReferenceSubscriptions` snapshot + alloc** — *What:* allocations per notify and `InvalidOperationException` resistance when a subscriber calls `Subscribe`/`Unsubscribe` mid-notify. *Test:* `Tests/Performance/NotifyBenchmarks.cs::Notify_50Subs_HalfUnsubscribeInline` for perf, plus a separate `[Test]` regression for the InvalidOperationException case. *Scenario:* 50 subscribers, half `Unsubscribe` themselves on first notify, `iterationsPer: 100000`. *Baseline:* current code allocates `List<T>.Enumerator` boxed via `IEnumerator<>` per notify (§4.11) and **throws** under inline-unsubscribe. *Success:* zero alloc per notify (struct enumerator over snapshot list), zero exceptions, ns/op within 10 % of the non-mutating baseline.
- **`StoreVariableStorage.SameTypedPayloadValues` typed equality switch** (entities.states bridge — measured here because the perf cost is amortised into states' notify path) — *What:* time per `ApplyCanonicalUpdate` + alloc for `EqualityComparer<VariableValue?>.Default.Equals` vs the six-method match chain. *Test:* `Tests/Performance/VariableEqualityBenchmarks.cs` in the entities.states package. *Scenario:* 50 variables, `iterationsPer: 10000`, each iteration touching one variable. *Baseline:* six virtual calls + boxing for value-typed `VariableValue` payloads. *Success:* single virtual call, ≥4× speedup, zero alloc once `VariableValue` is a `record` or `IEquatable<>`.
- **`Variables` `OrderBy` per read** (entities.states bridge §4.20; symptom appears here as states-API friction) — *What:* alloc per `Variables` getter call. *Test:* `Tests/Performance/VariablesEnumerationBenchmarks.cs`. *Scenario:* `iterationsPer: 10000` getter reads with no underlying state change; pair with `NoAllocations` post-fix. *Baseline:* `OrderBy` LINQ enumerator allocation per call. *Success:* zero alloc per read after §7.3 (cached sorted list), invalidated only on `CopyMapsIntoCaches`.
- **Hand-rolled type switch vs `EqualityComparer<T>.Default`** — covered by the variable-equality benchmark above; the same harness also compares `RegisteredMutator.Apply`'s `executeReference.Equals(Reference.Null)` (§4.13) vs `ReferenceEquals`. *Test:* `Tests/Performance/ReferenceEqualityBenchmarks.cs`. *Scenario:* `iterationsPer: 1000000`, mixed `Reference.Null` and custom `IReference` with overridden `Equals`. *Success:* `ReferenceEquals` ≥10× faster, identical correctness under a deliberately-misbehaving `Equals` override.
- **`Subscribe` closure allocation per call** — *What:* alloc per `Store.Subscribe<TState>(reference, action)` invocation. *Test:* `Tests/Performance/SubscribeBenchmarks.cs`. *Scenario:* `iterationsPer: 10000`, each iteration with a fresh lambda capturing one local. *Baseline:* one delegate alloc per call (unavoidable for a captured-lambda). *Success:* track only the *additional* allocations inside `Ledger.Add` (§4.10's `ContainsKey + indexer`); after the `TryGetValue` fix, zero extra alloc beyond the delegate itself.
- **`MutatorRegistry.TryGet` lookup + `IPayloadMutatorBinding` enumeration** — *What:* lookup time and zero-alloc proof for the dispatch decision. *Test:* `Tests/Performance/MutatorRegistryBenchmarks.cs` + a companion `Bench.NoAllocations` proof. *Scenario:* registry preloaded with 50 payload types, `iterationsPer: 100000` `TryGet` calls cycling through known and unknown keys. *Baseline:* `Dictionary<Type, List<>>` lookup + `Count == 0` guard. *Success:* zero alloc per lookup, ns/op within Dictionary baseline; serves as the "before" measurement for the §7.1 source-generated dispatcher.
