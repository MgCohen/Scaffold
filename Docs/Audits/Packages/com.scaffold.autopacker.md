...
- **`AddressableAssetSettings` + Unity 2.x catalog API native** — `https://docs.unity3d.com/Packages/com.unity.addressables@2.9/api/UnityEditor.AddressableAssets.Settings.AddressableAssetSettings.html`. **Verdict: Adopt for the build pipeline.** Unrelated to packing per se, but if the goal of `[AutoPack]` is to feed Addressables-shipped content snapshots into runtime, the native settings API is the boring path that doesn't require maintaining a Roslyn generator.
- **Unity AssetGraph** (`https://github.com/Unity-Technologies/AssetGraph`) — node-based asset processing pipeline. **Verdict: Steal pattern.** If autopacker grows into "transform asset bundles into shippable packed snapshots", AssetGraph is the prior-art for declarative pipeline-as-graph and avoids hand-rolling the generator-emit machinery.
- **BetterStreamingAssets** (`https://github.com/gwiazdorrr/BetterStreamingAssets`) — fast, allocation-free streaming asset reader on Android/iOS. **Verdict: Adopt** if the packed snapshots ever get loaded from `StreamingAssets`. Doesn't replace the generator, it complements it.
- **MessagePack-CSharp `[MessagePackObject]` + `MessagePackGenerator`** (`https://github.com/MessagePack-CSharp/MessagePack-CSharp`) — production-grade, IL2CPP-friendly source-generator-driven serializer. **Verdict: Adopt** if AutoPacker's mission is "blittable snapshots for IPC/persistence." MessagePack already solves: deterministic schema, version tolerance, and zero-reflection. Building a parallel generator for the same problem is a maintenance liability the audit's findings (binary blobs in repo, no rebuild pipeline, 0 production users) reinforce.
- **MemoryPack** (`https://github.com/Cysharp/MemoryPack`) — Cysharp's zero-encoding, zero-allocation, source-generated binary serializer. **Verdict: Steal pattern or Adopt outright.** If the only reason `[AutoPack]` exists is "I want a `Packed` struct per type with no reflection", MemoryPack is exactly that, with Unity-supported NuGet, and it's actively maintained. Strongly consider deleting `com.scaffold.autopacker` and pointing consumers at MemoryPack unless there's a packing semantic (e.g. lossy field mapping via `[Packed(typeof(T))]`) that MemoryPack genuinely can't express.

## 11. Benchmark plan

- **Smoke: generator emitted IL count.** Tool: NUnit EditMode (`AutoPackerTests.cs` companion). Test location: `Tests/AutoPackerGeneratorPresenceTests.cs`. Scenario: load `Assembly.Load("Scaffold.Autopacker.Tests").GetTypes()`; count types whose nested `Packed` exists; assert `≥ 3` (the three `[AutoPack]` types in the fixture). Baseline expectation: today, with LFS pulled correctly, 3 nested `Packed` types are present. Success criteria: **fails with a clear message** ("Generator did not emit Packed types — check that AutoPackerGenerator.dll is the LFS-resolved binary, not a 130-byte pointer") if LFS isn't fetched. This is the single most important test to add.
- **Smoke: DLL is not an LFS pointer.** Tool: NUnit EditMode (or even a CI bash check). Test location: `Tests/AutoPackerDllSanityTests.cs`. Scenario: open `Runtime/AutoPackerGenerator.dll` and assert the first two bytes are `0x4D 0x5A` (PE header `MZ`), not `version https...`. Costs nothing; turns "build mysteriously fails on a fresh clone" into one EditMode test failure with an actionable message.
- **Generator output determinism.** Tool: `dotnet publish Generators/AutoPacker -c Release` + `sha256sum`, run in CI. Test location: a CI job, not a Unity test. Scenario: rebuild twice in a clean container, diff the published DLLs. Baseline expectation: deterministic output; any drift is a generator bug. Success criteria: hashes match across two clean builds.
- **Sample build-time impact.** Tool: Unity batchmode build with and without `Scaffold.Autopacker` referenced from a target asmdef. Test location: a `Tools/build-bench.ps1` or CI matrix. Scenario: same set of 5 / 50 / 500 `[AutoPack]` types in a synthetic asmdef, measure compile time. Baseline expectation: ≤ 50ms overhead at 5 types, ≤ 500ms at 500. Success criteria: linear in type count, no super-linear hotspot.
- **Pack/unpack micro-bench.** Tool: Unity.PerformanceTesting. Test location: `Tests/Performance/AutoPackerRoundTripBench.cs`. Scenario: round-trip 10 / 100 / 10k `PlayerState` instances through `Pack(handler)` + `new PlayerState(packed)`. Baseline expectation: zero allocations on the hot path (the `Packed` struct is unmanaged, the handler should be ref-cached). Success criteria: 0 GC.Alloc per round-trip after warm-up; if `DefaultPackingHandler.Resolve<,>` shows allocations, audit 4.7 is the culprit and the handler needs `Convert.ChangeType` replaced with typed extension methods.
- **`Pack()` parameterless overload safety.** Tool: NUnit EditMode. Test location: `Tests/AutoPackerDefaultHandlerSafetyTests.cs`. Scenario: build a `[AutoPack]` type with a `[Packed(typeof(T))]` field where `T` is incompatible with `Convert.ChangeType` (e.g. `Vector3` → `int`); call `Pack()` with no handler; assert it throws **with a useful message** (audit 4.9). Baseline expectation: today, a raw `InvalidCastException` from `Convert.ChangeType` with no context. Success criteria after fix: a `PackingException` carrying the source type, target type, and field name.
