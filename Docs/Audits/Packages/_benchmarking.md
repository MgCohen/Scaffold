...
- **CI:** `unity -batchmode -runTests … -perfTestResults results.json` (and `-testResults` for NUnit XML). This repo’s scaffold perf microbenchmarks use NUnit **`PerformanceBenchmark`** (see `Assets/Benchmarks/**`). Default **`run-unity-tests.ps1`** passes **`-testCategory "!PerformanceBenchmark"`** so ordinary gates stay fast; omit that (use **`-IncludePerformanceBenchmarks`** or **`-PerformanceBenchmarksOnly`**) when collecting JSON. Feed JSON to [PerformanceBenchmarkReporter](https://github.com/Unity-Technologies/PerformanceBenchmarkReporter) for over-time HTML reports.
