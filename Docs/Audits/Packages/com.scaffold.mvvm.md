...
Path: `Assets/Packages/com.scaffold.mvvm`
Asmdef: `Scaffold.MVVM` (`noEngineReferences: false`).
Source generator authoring lives outside the package at `Generators/MVVMCompositionGenerator/MVVMCompositionGenerator.cs`; the compiled DLLs are shipped in-package under `GeneratorsMVVM/`.
...
- Generator authoring source (`Generators/MVVMCompositionGenerator/MVVMCompositionGenerator.cs`) lives outside the package; the compiled DLLs ship inside (`GeneratorsMVVM/Composition/MVVMCompositionGenerator.dll`). Document the build pipeline in the package README — right now it's invisible to consumers.
...
Repo-wide grep (`Assets/`, `GameModule/`, `LiveOps/`) for `using Scaffold.MVVM*` and the package's public symbols (`[BindSource]`, `[NestedObservableObject]`, `BindedProperty`, `BindedCollection`, `TreeBinding`, `DeferredBindingCoroutineHost`) excluding the package itself. **Result: a single non-test consumer.** The MVVM engine is shipped but essentially unused outside the trio of MVVM/View/ViewModel packages.

- `Assets/Packages/com.scaffold.model/Runtime/Model.cs:6` — sole `[NestedObservableObject]` usage outside the MVVM packages. The class is empty (`public partial class Model : ObservableObject`), so the generator emits a no-op `RegisterNestedProperties` per derived class. Reinforces issue 4.16 in this audit (per-derivation duplicate emission): every concrete model gets one extra generated file with `base.RegisterNestedProperties()` and nothing else. Smell visible from this call site: the attribute is mandatory boilerplate to participate in the binding tree, not a feature decision — `Model` doesn't choose to be observable, it has to be.
- `Assets/Packages/com.scaffold.view/Samples/MVVMUseCases.cs:73-86` — `SampleViewModel : ViewModel` with `[ObservableProperty] private BuildSampleModel sampleModel; [ObservableProperty] private int value;` and `Bind(() => SampleModel.Value, () => Value)` in `Initialize`. This is the only typed `Bind(() => ..., () => ...)` call site in the repo. Smell: the lambdas are typed but the source-property name (`SampleModel.Value`) collides with the target property name (`Value`) by string path — if anyone renames `SampleModel`, the registry path changes silently (issue 4.7).
- `Assets/Packages/com.scaffold.view/Samples/MVVMUseCases.cs:32-39` — `BuildRegisterNested(viewModel)` calls `nested.RegisterNestedProperties()` directly. The SCM003 analyzer flags manual `RegisterNestedProperties` invocation; the sample is the analyzer's negative example, but it lives in shipped sample code. Confusing.
- `[BindSource]` direct usages: **zero outside the MVVM trio.** The attribute is documented as the "tag your VM/View" entry point, but consumers exclusively reach it transitively by deriving `ViewModel` (which has `[BindSource(typeof(TreeBinding))]`) or `ViewElement<T>`. So `[BindSource]` is effectively a private implementation detail; the public surface is "derive from `ViewModel` / `ViewElement<T>`."
- `BindedProperty<T>` direct uses: **zero.** All access is through the generator-emitted `Bind/BindCollection/UpdateBinding` facade. This is good (the type is not part of the consumer-facing API), but it also means the dead code paths flagged in 4.3 (`TryConvertFallback`), 4.10 (no-arg `Update()`), and 4.14 (post-hoc `WithConverter`) have **no production callers** — they can be deleted with zero migration cost.
- `BindedCollection<T>` direct uses: **zero.** Same conclusion. The `INotifyCollectionChanged` re-subscription logic in 4.13 is theoretically broken but has no field exercise.
- `TreeBinding` direct construction: **zero outside the package.** The generator instantiates it.
- `DeferredBindingCoroutineHost` references: **zero outside the package.** The host is wired internally; consumers never name it. This means R1 (extract engine to a Unity adapter assembly) is a near-mechanical refactor — no consumer code needs to change.
- `UpdateBinding(string)` calls: only `ViewModel.OnPropertyChanged` (`com.scaffold.viewmodel/Runtime/ViewModel.cs:54-58`) and `ViewElement.OnViewModelChanged` (`com.scaffold.view/Runtime/ViewElement.cs:23-25`) — both internal to the trio. The stringly-typed dispatch never crosses the package boundary.

**Net read on the existing audit findings.** The findings stand, but their *severity* shifts with this consumer evidence:
- The `BindedProperty.Update` exception-swallow (4.2) is a latent timebomb — no consumer has hit it because nobody is configuring custom converters in production yet. When they do, it'll silently eat their bugs.
- The `noEngineReferences: false` boundary leak (4.1) and `MVVMCompositionGenerator` per-derivation emission (4.16) are easier to fix than expected because there is essentially no third-party code to migrate. The architect can be aggressive here.
- The expression-tree binding API (R2) is being used in exactly one place (the sample). If the team wants to swap to R3 or compiled `[ObservableProperty]` callbacks, the migration cost today is one file. **This window closes the moment the game team ships features.**

## Alternatives & prior art

| Library / pattern | Description | Verdict | Rationale |
|---|---|---|---|
| **CommunityToolkit.Mvvm** ([`[ObservableProperty]`, `[RelayCommand]`](https://learn.microsoft.com/dotnet/communitytoolkit/mvvm/)) — DLL already in `GeneratorsMVVM/Community/` | Microsoft's MVVM source generator shipping with `ObservableObject`, `[NotifyPropertyChangedFor]`, `[RelayCommand]`. | **Adopt deeper.** | `ObservableObject` is already the base of `ViewModel`. `[RelayCommand]` is shipped but unused — adopt it for view-to-VM commanding (currently routed through `ViewEvents`). Removes need to roll your own command pattern. |
| **Cysharp R3** ([github.com/Cysharp/R3](https://github.com/Cysharp/R3)) | Modern Rx for .NET / Unity. Per-property `Observable<T>` with `Subscribe`, `AddTo(MonoBehaviour)` lifetime ties. | **Steal pattern.** | Replace `Expression<Func<T>>` registration + string path with a typed `Observable<T>` per `[ObservableProperty]`. Eliminates `BindGroups`, `BindingPath`, the registry, and the entire string-keyed dispatch (issue 4.7). Generator change only; consumers' `Bind(() => x, ...)` syntax is preservable as a thin wrapper. |
| **WeakEventManager** ([.NET WPF docs](https://learn.microsoft.com/dotnet/desktop/wpf/advanced/weak-event-patterns)) | Pattern that subscribes to events through a weak-reference manager so the publisher cannot keep the subscriber alive. | **Wrap.** | Direct fix for the View-side INPC subscription leak (sibling package, issue 4.1). For the engine itself, `BindContext` doesn't subscribe weakly to `INotifyPropertyChanged`; if a `ViewModel` outlives a binding consumer, the consumer leaks. Today the symmetric `Dispose` mostly hides this; an opt-in weak path future-proofs it. |
| **Avalonia / MAUI compiled bindings** ([Avalonia docs](https://docs.avaloniaui.net/docs/data-binding/compiled-bindings)) | XAML `x:CompileBindings` produces typed get/set delegates at compile time, no runtime expression interpretation. | **Steal pattern.** | Validates the architect's "compile-time-typed" preference. The `MVVMCompositionGenerator` already emits typed `Bind<TSource,TTarget>` wrappers — extend it to emit per-property `BindFoo(Action<int>)` methods, dropping the `Expression<Func<T>>` dependency entirely. |
| **MessagePipe + VContainer** ([github.com/Cysharp/MessagePipe](https://github.com/Cysharp/MessagePipe)) | DI-scoped pub/sub; `IPublisher<T>` / `ISubscriber<T>` with auto-disposal on scope death. | **Wrap.** | Not for the binding engine itself (out of scope) but for the deferred update queue currently coupled to a hidden `MonoBehaviour` host (`DeferredBindingCoroutineHost`). A scoped `IPublisher<DeferredBindKey>` removes the static `GameObject` allocation and ties lifetime to the LifetimeScope. |

## Benchmark plan

- **`BindedProperty<T,U>.Update` allocation/throughput.** Measure: per-call alloc bytes, ns/op. Tool: `Unity.PerformanceTesting` `[Performance]` test in PlayMode. Location: `Assets/Packages/com.scaffold.mvvm/Tests/PerfBindedPropertyUpdate.cs` (new). Scenarios: `int→int` (no convert), `int→string` (converter path), `int→string` (TryConvertFallback path). Baseline: < 32 B/alloc per `int→int` update; current likely boxes through `TryConvertFallback`. Success: zero alloc on the no-converter path; deletion of `TryConvertFallback` (4.3) should eliminate allocs on the `is TTarget` path.
- **`BindedCollection<T,U>` change-notification cost.** Measure: ns and alloc for `Add`, `Remove`, `Reset` on collections of 10/100/1000 items. Tool: `Unity.PerformanceTesting`. Location: `Assets/Packages/com.scaffold.mvvm/Tests/PerfBindedCollection.cs`. Baseline: linear in collection size for `Reset`; constant for `Add/Remove`. Success: confirm no regressions when 4.13 is tightened (track currently-subscribed `INotifyCollectionChanged` explicitly).
- **`UpdateBinding(string)` lookup path.** Measure: ns/op on a `TreeBinding` with 1, 10, 100 registered groups, dispatching by hot vs cold key. Tool: `Unity.PerformanceTesting`. Location: `Assets/Packages/com.scaffold.mvvm/Tests/PerfUpdateBinding.cs`. Scenario: cold-cache string hash + `Map<string,Type,IBindContext>` lookup. Baseline: ≈ 50-100 ns/lookup at 100 keys. Success: after caching `(Type, propertyName) → int` (R3 in audit §7), constant-time without string hashing in the steady state.
- **`UpdateBind` "miss creates group" leak proof.** Test (correctness, not perf): `EditMode` test calling `treeBinding.UpdateBind("Bogus.Path")` 1,000× and asserting `groups.Count == 0` after. Tool: NUnit. Location: `Assets/Packages/com.scaffold.mvvm/Tests/UpdateBindMissTests.cs` (new). Today this test would **fail** — that's the bug in issue 4.5.
- **`MVVMCompositionGenerator` compile-time throughput.** Measure: generator wall-time for VMs with 1 / 5 / 20 `[ObservableProperty]` fields and 0 / 1 / 5 nested `[NestedObservableObject]` children; emitted IL bytes. Tool: `Microsoft.CodeAnalysis.Testing` (`CSharpSourceGeneratorTest`) snapshot tests. Location: `Generators/MVVMCompositionGenerator.Tests/GeneratorPerfTests.cs` (new). Baseline: snapshot the current outputs. Success: post-R2 (typed-callback emission, no string paths), emitted file count drops; per-property method count predictable; no regression in generator wall-time at p95.
- **Dead-code sanity benchmark.** With `BindedProperty.TryConvertFallback`, `BindedCollection.Update()` no-arg, and `BindSets.RegisterAdapter` removed (audit §6 easy wins), re-run the alloc bench above. Success: identical or better numbers (proves they were dead) and reduced JIT footprint.
