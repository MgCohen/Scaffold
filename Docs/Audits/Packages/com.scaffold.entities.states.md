...
All scenarios use **Unity.PerformanceTesting**. Tests live under `Assets/Packages/com.scaffold.entities.states/Tests/Performance/` (new folder), gated by an asmdef define constraint so the unit suite stays fast. The states-package benchmarks (`Store.Execute`, `Store.EnumerateAll`, notify) are referenced from this package's plan only when the entities bridge is the dominant consumer.

- **`StoreVariableStorage.ApplyCanonicalUpdate` rebuild cost (§4.9, §4.10)** — *What:* allocations and time per `EntityVariableState` mutation arriving at `OnStateChanged`. *Tool:* Unity.PerformanceTesting + `GC.GetAllocatedBytesForCurrentThread()` delta. *Test:* `Tests/Performance/ApplyCanonicalUpdateBenchmarks.cs::Update_50Vars_OneChanged`. *Scenario:* entity with 50 variables, one modifier change per iteration, 10⁴ iterations. *Baseline:* current code allocates 1× `HashSet<Variable>` (`ComputeKeySet`) + 1× `Dictionary<Variable, VariableValue>` (`BuildCurrentValueMap`) + a redundant union walk (§4.10), per change. *Success:* after §7.1 (incremental diff), allocations per change drop to zero on the steady-state path; ns/op ≥5× faster on the "one of 50 variables changed" scenario.
- **`StoreVariableStorage.SameTypedPayloadValues` typed equality switch (§4.7)** — *What:* per-call time and alloc for the equality decision. *Tool:* Unity.PerformanceTesting. *Test:* `Tests/Performance/VariableEqualityBenchmarks.cs::Equals_FloatVariableValue` and `Equals_StringVariableValue` and `Equals_UnknownTypeFallthrough`. *Scenario:* 10⁶ comparisons across each `VariableValue` type plus an unknown subtype. *Baseline:* up to six virtual `MatchXxxEquals` calls + boxing. *Success:* one `EqualityComparer<VariableValue?>.Default.Equals` call, ≥4× speedup, **and** the unknown-type case correctly returns equality (today it falls through to `ReferenceEquals` and silently spuriously notifies — §4.7).
- **`Variables` enumeration with `OrderBy` (§4.20)** — *What:* alloc per `Variables` getter call when nothing changed. *Tool:* Unity.PerformanceTesting + alloc delta. *Test:* `Tests/Performance/VariablesEnumerationBenchmarks.cs::Variables_RepeatedReads_NoChange`. *Scenario:* 10⁴ reads of `storage.Variables` on a 50-variable entity, no underlying changes. *Baseline:* `OrderBy` LINQ enumerator alloc + `StringComparer.Ordinal` per call. *Success:* zero alloc per read after caching the sorted list (entities.states §7.3), invalidated only on `CopyMapsIntoCaches`.
- **`StoreVariableStorage.NotifyVariableSubscribers` defensive copies (§4.8)** — *What:* per-notify alloc count when subscribers don't mutate the subscription list. *Tool:* Unity.PerformanceTesting. *Test:* `Tests/Performance/NotifyVariableBenchmarks.cs::Notify_50Vars_NoSubscriberChurn`. *Scenario:* entity with 50 variables, 10 subscribers per variable, 10⁴ canonical updates each touching one variable. *Baseline:* `perVariableSubscribers.Keys.ToList()` + per-variable `list.ToArray()` + `structuralSubscribers.ToArray()` per notify. *Success:* zero alloc per notify after the states-package §4.11 fix is consumed (snapshot-once at the `IStateEventHandler` boundary, ping-pong buffers internally).
- **`StateEntityOps.RemoveModifiersFromSource` cross-entity sweep** — *What:* time + alloc for the global sweep across N entities, M of which carry a modifier from `source`. *Tool:* Unity.PerformanceTesting. *Test:* `Tests/Performance/StateEntityOpsBenchmarks.cs::RemoveModifiersFromSource_1000Entities_100Affected`. *Scenario:* 1000 entities, 100 of them with one modifier from `auraSource`, 10³ sweeps. *Baseline:* `store.EnumerateAll<EntityVariableState>()` boxed enumerator (states §4.6) + `List<object>` payload list + per-slice `IReadOnlyList<ActiveModifier>` linear scan (`HasAnyModifierFromSource`). *Success:* after states §7.3 (per-type indexed bucket) + payload `List<object>` rented from a pool, alloc per sweep drops to one rented list; ns/op ≥3× faster.
- **`EntityVariableState.ResolveEffectiveValues` cost (§4.10)** — *What:* time + alloc per resolve on a steady-state entity. *Tool:* Unity.PerformanceTesting. *Test:* `Tests/Performance/ResolveEffectiveValuesBenchmarks.cs::Resolve_50Vars_5ModifiersEach`. *Scenario:* 50 variables, 5 modifiers per variable, 10⁴ resolves. *Baseline:* one `HashSet<Variable>` (`CollectKeysUnion`) + one result dictionary per call. *Success:* zero `HashSet` alloc after the union is folded into the same loop as the result (eliminate §4.10's redundant walk); per-call alloc reduced to one result dictionary, eliminable entirely if the resolver writes into a caller-supplied buffer.
- **`StateEntity.AddModifier` / `RemoveModifier` round-trip** — *What:* end-to-end alloc + ns/op for the public mutating API consumers will actually call. *Tool:* Unity.PerformanceTesting. *Test:* `Tests/Performance/StateEntityRoundTripBenchmarks.cs`. *Scenario:* one entity, one variable, 10⁴ alternating `AddModifier`/`RemoveModifier`. *Baseline:* one payload record alloc per call (record class), plus the states-package execute path costs. *Success:* serves as the **integration baseline** for the entire package; track regressions against this number across refactors. Target: ≤1 record alloc per call after §7.4 (source-gen registration removes registry lookups).
