...
No view, viewmodel, liveops, gameplay assembly, GameModule, or LiveOps server code currently depends on this package. The only "consumer" is `Tests/StateEntityIntegrationTests.cs`, which is the closest thing to a real-world usage example. **This is itself the most actionable consumer finding: the package's external consumers do not yet exist, so every API decision should be driven by the test file as the canonical adoption template.**

Reading the tests as a consumer:

- `Tests/StateEntityIntegrationTests.cs:73, 458, 489, 574` — every test rebuilds the wiring boilerplate manually:
  ```csharp
  var store = new StoreBuilder().Build();
  EntityBridgeContext.RegisterMutators(store);
  ```
  Two lines that always go together. **Smell: there is no factory for "store + entity bridge mutators registered."** A `EntityBackedStoreBuilder` (or a `StoreBuilder.WithEntityBridge()` extension) would collapse this. The fact that every test repeats it is a strong "the API is one helper short" signal.
- `Tests/StateEntityIntegrationTests.cs:506–508` — three structurally identical subscriptions, each discarding two of three callback args:
  ```csharp
  store.Subscribe<EntityVariableState>(heroId, (_, _, ev) => { if (ev == StateChangeEvent.Updated) { heroUpdates++; } });
  ```
  Every consumer subscription to a *keyed* state has the same shape: throw away `IReference` and `TState`, branch on `StateChangeEvent`. The states-package callback signature is over-broad for this case. (Cross-referenced in `com.scaffold.states.md` Consumers section.)
- `Tests/StateEntityIntegrationTests.cs:21, 36–37, 47, 56, 88, 101, 112, 466–468` — every mutation goes through `store.Execute(id, new SomePayload(id, ...))` where `id` is passed twice: once as the slice key, once as the payload's `EntityId` field. **Smell: payload duplicates the routing key.** The payload records (`AddModifierPayload`, `RemoveModifierPayload`, `SetBaseValuePayload`, `AddEntityVariablePayload`, `RemoveEntityVariablePayload`, `RemoveModifiersBySourcePayload`) all carry an `InstanceId EntityId` that's redundant whenever the consumer also supplies the reference. Either drop `EntityId` from the payload (use the routing reference) or drop the `id` parameter from `store.Execute(id, payload)` (use payload's `EntityId`). Today both must be kept in sync — TOCTOU surface.
- `Tests/StateEntityIntegrationTests.cs:473, 510` — `StateEntityOps.RemoveModifiersFromSource(store, auraSource)` — a static helper. Consumers must remember the class name. It would be discoverable as `store.RemoveModifiersFromSource(auraSource)` (extension) or as a method on `StateEntity` if a per-entity variant is needed. Static class with one method is friction.
- `Runtime/StateEntityOps.cs:39–42` — `if (reference is not EntityStateReference entityRef) { return; }` — the package's own internal cross-entity sweep performs a manual cast on a key that, by the package's own invariant, must be `EntityStateReference`. If consumers later want to add their own `EntityVariableState`-shaped slice (legal under the current `Store` API), this filter will silently skip it. This is the consumer-side mirror of states audit §4.15: typed slice keys are the fix.
- `Runtime/StoreVariableStorage.cs:24` — `store.Subscribe<EntityVariableState>(entityId, onStateChangedHandler)` — the bridge package itself uses the over-broad 3-arg callback (`Action<IReference, EntityVariableState, StateChangeEvent>`) only to read the third arg in `OnStateChanged` (line 19). Even the *internal* consumer pays the "I only care about `StateChangeEvent`" tax. Confirms the over-broad-callback finding.
- `Runtime/StoreVariableStorage.cs:30–34` — `lastKeySnapshot.OrderBy(v => v.Key, StringComparer.Ordinal)` — the bridge package itself takes a defensive `OrderBy` allocation per `Variables` read. A view layer iterating an entity's variables every frame would compound this; a consumer in a future MVVM binding would need to wrap and cache this anyway.
- `Runtime/StoreVariableStorage.cs:239, 252, 272, 280` — `.ToArray()` and `.ToList()` defensive copies on every notify. This is the bridge defending itself against re-entrancy because the underlying `IStateEventHandler` doesn't snapshot subscribers (states §4.11). The bridge essentially **re-implements the snapshot-subscriber pattern that the states package should provide**. Once states §4.11 is fixed, all four of these defensive copies become deletable.
- `Runtime/EntityBridgeContext.cs:14–19` — six hand-edited `store.RegisterMutator(new XxxMutator())` lines. Every new payload+mutator pair adds a 7th, 8th, … line that consumers must remember. Source-gen registration (entities.states §7.4) eliminates this.
- `Runtime/StoreInstanceIdExtensions.cs:10–53` (covered fully in the states-package Consumers section) — eight extensions wrapping `Store.X(IReference, …)` as `Store.X(InstanceId, …)`. The strongest "states-package API needs typed keys" signal in the codebase.

Cross-cutting consumer findings specific to entities.states:

- The package's own internal storage (`StoreVariableStorage`) re-implements three things the states package should provide: subscriber-snapshot (against re-entrancy), per-call value-equality dispatch, and keyed enumeration with stable order. Each is a missing primitive in `Scaffold.States`.
- Payloads carry redundant identity. Removing `EntityId` from payload records (use the routing `IReference` instead) shrinks the API and removes a TOCTOU class.
- The factory pair `EntityStateFactory.Create(...) + EntityBridgeContext.RegisterMutators(store)` is two ceremonies for one outcome ("I want an entity backed by a store"). One factory call should suffice.

## Alternatives & prior art

- **Fluxor `[ReducerMethod]` source generator. Verdict: Steal pattern.** This package has **six** payload+mutator pairs registered manually in `EntityBridgeContext.RegisterMutators`. Fluxor's attribute-driven, source-gen'd reducer registration is a 1:1 fit. Adding a 7th pair (e.g. a `SetVariableMetadataPayload`) currently requires touching `EntityBridgeContext` — with Fluxor's pattern it would auto-register. (https://github.com/mrpmorris/Fluxor)
- **MessagePipe (Cysharp) — typed pub/sub with VContainer integration. Verdict: Wrap (events only).** `StoreVariableStorage.NotifyVariableSubscribers` and `NotifyStructuralDiffs` are hand-rolled MessagePipe. A typed `IPublisher<EntityVariableChangedEvent<TVariable>>` would replace `perVariableSubscribers` (a `Dictionary<Variable, List<Action<VariableValue>>>`) with a typed channel and remove the `.ToArray()` defensive copies (§4.8). Don't replace the canonical state itself — the `Store` is the source of truth — but the per-variable fan-out is a clean swap. (https://github.com/Cysharp/MessagePipe)
- **R3 (Cysharp) — `ReactiveProperty<T>`, `Observable<T>`. Verdict: Wrap (consumer-facing).** `StoreVariableStorage.Subscribe(Variable, Action<VariableValue>)` and the structural-change subscription would benefit from R3's `IDisposable` subscription model and built-in value deduplication (eliminates `SameTypedPayloadValues` — §4.7 — entirely, since `ReactiveProperty<T>` filters duplicates via `IEqualityComparer<T>`). For a future view/viewmodel consumer that doesn't yet exist, exposing `IEntityVariableStorage.Observe(Variable) : Observable<VariableValue>` would be more idiomatic than the current `Subscribe(...) → IDisposable` pair. (https://github.com/Cysharp/R3)
- **Unity DOTS / ECS — `IComponentData`, `EnableableComponent`, change filters. Verdict: Steal pattern.** `EntityVariableState` is morally an `IComponentData`; `AddModifierMutator` is morally an `ISystem`. ECS pays nothing per dispatch because of generic specialization, and change filtering on chunks gives O(changed) notify, not O(subscribers). The relevant pattern to steal: keep a per-slice `version : ulong` and let `StoreVariableStorage` early-out when `state.Version == lastSeenVersion`. Combined with §7.1 (incremental diff), this gives a near-zero-allocation hot path for ticking modifiers. (https://docs.unity3d.com/Packages/com.unity.entities@latest)
- **Source-generated dispatchers (MagicOnion service registry, Spectre.Console.Cli). Verdict: Steal pattern.** Same as the states audit; specifically applied here, the six payload+mutator pairs become attribute-tagged: `[MutatedBy(typeof(AddModifierMutator))] public sealed record AddModifierPayload(...)`. Generator emits `EntityBridgeContext.RegisterMutators` from compile-time discovery. (https://github.com/Cysharp/MagicOnion)
- **AutoMapper — Skip.** `EntityVariableState.ResolveEffectiveValues` projects modifier stacks → effective `Dictionary<Variable, VariableValue>`. AutoMapper would reflectively wire properties; this projection is loop-driven over modifiers in order, which AutoMapper doesn't model well. Stay hand-rolled; if anything, a custom source-gen mapper (see §4.7 / §7.1) is the right direction.

## Benchmark plan

All scenarios use **Unity.PerformanceTesting** + the shared `Bench.Measure(...)` helper. See [`_benchmarking.md`](./_benchmarking.md) for the helper code, run policy (Editor + PlayMode IL2CPP), and the project-wide pass/fail thresholds. Tests live under `Tests/Performance/` (new folder) with a dedicated asmdef gated by define constraint so the unit suite stays fast. Cross-package benchmarks in the states package's plan (`Store.Execute`, `Store.EnumerateAll`, notify) are referenced from here only when the entities bridge is the dominant consumer. Pair every "zero allocations" claim with a companion `[Test] Bench.NoAllocations(() => ...)` so a regression hard-fails the PR.

- **`StoreVariableStorage.ApplyCanonicalUpdate` rebuild cost (§4.9, §4.10)** — *What:* allocations and time per `EntityVariableState` mutation arriving at `OnStateChanged`. *Test:* `Tests/Performance/ApplyCanonicalUpdateBenchmarks.cs::Update_50Vars_OneChanged`. *Scenario:* entity with 50 variables, one modifier change per iteration, `iterationsPer: 10000`. *Baseline:* current code allocates 1× `HashSet<Variable>` (`ComputeKeySet`) + 1× `Dictionary<Variable, VariableValue>` (`BuildCurrentValueMap`) + a redundant union walk (§4.10), per change. *Success:* after §7.1 (incremental diff), allocations per change drop to zero on the steady-state path; ns/op ≥5× faster on the "one of 50 variables changed" scenario.
- **`StoreVariableStorage.SameTypedPayloadValues` typed equality switch (§4.7)** — *What:* per-call time and alloc for the equality decision. *Test:* `Tests/Performance/VariableEqualityBenchmarks.cs::Equals_FloatVariableValue` / `Equals_StringVariableValue` / `Equals_UnknownTypeFallthrough`. *Scenario:* `iterationsPer: 1000000` comparisons across each `VariableValue` type plus an unknown subtype. *Baseline:* up to six virtual `MatchXxxEquals` calls + boxing. *Success:* one `EqualityComparer<VariableValue?>.Default.Equals` call, ≥4× speedup, **and** the unknown-type case correctly returns equality (today it falls through to `ReferenceEquals` and silently spuriously notifies — §4.7).
- **`Variables` enumeration with `OrderBy` (§4.20)** — *What:* alloc per `Variables` getter call when nothing changed. *Test:* `Tests/Performance/VariablesEnumerationBenchmarks.cs::Variables_RepeatedReads_NoChange` + companion `NoAllocations` post-fix. *Scenario:* `iterationsPer: 10000` reads of `storage.Variables` on a 50-variable entity, no underlying changes. *Baseline:* `OrderBy` LINQ enumerator alloc + `StringComparer.Ordinal` per call. *Success:* zero alloc per read after caching the sorted list (entities.states §7.3), invalidated only on `CopyMapsIntoCaches`.
- **`StoreVariableStorage.NotifyVariableSubscribers` defensive copies (§4.8)** — *What:* per-notify alloc count when subscribers don't mutate the subscription list. *Test:* `Tests/Performance/NotifyVariableBenchmarks.cs::Notify_50Vars_NoSubscriberChurn` + companion `NoAllocations` post-fix. *Scenario:* entity with 50 variables, 10 subscribers per variable, `iterationsPer: 10000` canonical updates each touching one variable. *Baseline:* `perVariableSubscribers.Keys.ToList()` + per-variable `list.ToArray()` + `structuralSubscribers.ToArray()` per notify. *Success:* zero alloc per notify after the states-package §4.11 fix is consumed (snapshot-once at the `IStateEventHandler` boundary, ping-pong buffers internally).
- **`StateEntityOps.RemoveModifiersFromSource` cross-entity sweep** — *What:* time + alloc for the global sweep across N entities, M of which carry a modifier from `source`. *Test:* `Tests/Performance/StateEntityOpsBenchmarks.cs::RemoveModifiersFromSource_1000Entities_100Affected`. *Scenario:* 1000 entities, 100 of them with one modifier from `auraSource`, `iterationsPer: 1000` sweeps. *Baseline:* `store.EnumerateAll<EntityVariableState>()` boxed enumerator (states §4.6) + `List<object>` payload list + per-slice `IReadOnlyList<ActiveModifier>` linear scan (`HasAnyModifierFromSource`). *Success:* after states §7.3 (per-type indexed bucket) + payload `List<object>` rented from a pool, alloc per sweep drops to one rented list; ns/op ≥3× faster.
- **`EntityVariableState.ResolveEffectiveValues` cost (§4.10)** — *What:* time + alloc per resolve on a steady-state entity. *Test:* `Tests/Performance/ResolveEffectiveValuesBenchmarks.cs::Resolve_50Vars_5ModifiersEach`. *Scenario:* 50 variables, 5 modifiers per variable, `iterationsPer: 10000`. *Baseline:* one `HashSet<Variable>` (`CollectKeysUnion`) + one result dictionary per call. *Success:* zero `HashSet` alloc after the union is folded into the same loop as the result (eliminates §4.10's redundant walk); per-call alloc reduced to one result dictionary, eliminable entirely if the resolver writes into a caller-supplied buffer.
- **`StateEntity.AddModifier` / `RemoveModifier` round-trip** — *What:* end-to-end alloc + ns/op for the public mutating API consumers will actually call. *Test:* `Tests/Performance/StateEntityRoundTripBenchmarks.cs`. *Scenario:* one entity, one variable, `iterationsPer: 10000` alternating `AddModifier`/`RemoveModifier`. *Baseline:* one payload record alloc per call (record class), plus the states-package execute path costs. *Success:* serves as the **integration baseline** for the entire package; track regressions against this number across refactors. Target: ≤1 record alloc per call after §7.4 (source-gen registration removes registry lookups).
