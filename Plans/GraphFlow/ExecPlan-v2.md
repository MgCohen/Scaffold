...
`GraphPackageEmitter.EmitGenericNodeArtifacts` currently walks only the runner's containing assembly for `[GraphNode]` types. The M2 comment `"M2 supports a single source assembly per package; consumer-authored nodes in other referenced assemblies are a v2 follow-up"` is the line we delete.
...
- Update `GraphPackageEmitter.EmitGenericNodeArtifacts` to walk all asms that reference the package's runtime asm (D6).
