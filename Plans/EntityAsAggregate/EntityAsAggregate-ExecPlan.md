...
- [ ] Milestone 1 — `Scaffold.States` gains a runtime `Store.RegisterAggregate(IReference, IAggregateProvider)` method (and a regression test). After this milestone, an aggregate slice can be installed onto a Store after `Build()` returns.
- [ ] Milestone 2 — Bridge refactor: introduce `StateEntity<TDef> : AggregateState` (record) and `EntityStateProvider<TDef> : AggregateProvider<StateEntity<TDef>>` (provider class); shrink `EntityVariableState` to `(BaseValues, ModifierStacks)` and add behavior methods (`WithModifier`, `WithoutModifier`, `WithBaseValue`, `WithVariable`, `ResolveEffectiveValues`); mutators become one-line translations from payload to record method call; `EntityBridgeContext` collapses to a static `RegisterMutators(Store)` helper; `EntityStateFactory.Create` returns the initial `StateEntity<TDef>` record after registering source slice and aggregate provider. The bridge's existing class-based `StateEntity<TDef> : BaseEntityInstance<TDef>` is retired; gameplay code uses the record plus the `InstanceId` for store operations. `StoreVariableStorage` and `EffectiveValueRecomputer` are deleted. Tests adapted; new aggregate-rebuild-after-snapshot test added.
- [ ] Milestone 3 — `MutatorRegistry` dedupe: `Scaffold.States.MutatorRegistry.Register` throws `DuplicateMutatorRegistrationException` when called twice with the same `(TState, TPayload, mutator concrete runtime type)`. Independent safety improvement — the bridge's own usage pattern only registers each mutator type once, but this prevents the next consumer from tripping on the original dispatch foot-gun.
- [ ] Final validation gate — analyzer pass clean; EditMode tests for `Scaffold.States.Tests`, `Scaffold.Entities.Tests`, and `Scaffold.Entities.States.Tests` all green.
