...
- [ ] Milestone 1 — Land `com.scaffold.variables` package skeleton with `Variable`, `IVariableHandle`, `IReadOnlyVariableHandle<T>`, `IVariableHandle<T>`, `IVariableBag`, `VariableDefault` / `VariableDefault<T>`, `InMemoryHandle<T>`, `InMemoryVariableBag`. Plus `MutatorRegistry.IsRegistered(Type)` addition to `com.scaffold.states`. Pure additive — no consumer changes yet.
- [ ] Milestone 2 — Migrate GraphFlow. Delete its private variable types; replace `runner.CreateParentBag()` with `runner.CreateVariableBag(seed)`; migrate `cell.Value = x` call sites to `handle.Set(x)`. All existing GraphFlow tests must stay green.
- [ ] Milestone 3 — Migrate Entities. Extract `Variable` to shared. Move `payloadTypeId` from `Variable` to `VariableEntry`. Make `IEntityVariableStorage : Scaffold.Variables.IVariableBag`. Ship `[Obsolete]` extension routing `TryGetBase` → `TryGet<T>`. Write asset-migration script (`OnAfterDeserialize`) for old `.asset` files.
- [ ] Milestone 4 — Build `StoreVariableBagBuilder` + `StoreBackedHandle<TState, T>` in `com.scaffold.entities.states`. Includes the three-test deferral fixture and construction-time payload-registration validation.
- [ ] Milestone 5 — Erase the `[Obsolete]` extension methods. Codebase ends with exactly one `IVariableBag` interface.
- [ ] Milestone 6 — Wire a real consumer end-to-end. First sample graph runs against a state-backed bag, demonstrating snapshot round-trip preservation.

## Surprises & Discoveries

To be filled as work proceeds. None yet.
...
To be written at completion of each milestone and at the end of the plan.
...
The goal of this milestone is to make graphflow use only `Scaffold.Variables` types. Per Invariant 1, graphflow's existing `Scaffold.GraphFlow.IVariableBag`, `VariableCell<T>`, `InMemoryVariableBag`, and `VariableDefault` hierarchy under `Assets/Packages/com.scaffold.graphflow/Runtime/Variables/` are deleted, not aliased. Per Invariant 2, the `runner.CreateParentBag()` seam is replaced with `runner.CreateVariableBag(IEnumerable<RuntimeVariable> seed)`.

The deletion targets are: `Assets/Packages/com.scaffold.graphflow/Runtime/Variables/IVariableBag.cs`, `VariableCell.cs`, `InMemoryVariableBag.cs`, `VariableDefault.cs`. After deletion, every `using Scaffold.GraphFlow;` that referenced these types updates to `using Scaffold.Variables;`. The `RuntimeVariable.defaultValue` field type changes from `Scaffold.GraphFlow.VariableDefault` to `Scaffold.Variables.VariableDefault`; concrete subclass references (`IntDefault` etc.) similarly update. Any `cell.Value = x` call site (in the runtime `SetVariableNode<T>` partials and in tests) migrates to `handle.Set(x)`. Reads (`cell.Value`) stay as `handle.Value` since the read property is preserved on `IReadOnlyVariableHandle<T>`. Subscriptions on cells (`cell.Changed += ...` in `ObserveVariableNode<T>`) migrate to `handle.Subscribe(...)`.

The seam change is the more involved part. `GraphRunner.CreateParentBag()` is removed. `GraphRunner.CreateVariableBag(IEnumerable<RuntimeVariable> seed)` replaces it as a `protected internal virtual` method returning `IVariableBag`. The default implementation is `=> new InMemoryVariableBag(MaterializeSeed(seed))` where `MaterializeSeed` constructs handles by calling `RuntimeVariable.defaultValue.CreateHandle(id)` for each entry. The internal `GraphRunner.SeedVariables(seed)` plumbing collapses — `GraphBuilder.Build` no longer constructs a bag with `new InMemoryVariableBag(seed, runner.CreateParentBag())`; instead it calls `runner.Variables = runner.CreateVariableBag(seed)` directly. The `Flow.Variables` per-Run scratch bag remains as-is (its `Parent` chains to `runner.Variables`).

A `WithFallback(IEnumerable<RuntimeVariable> seed, FallbackMode mode)` method lives on `StoreVariableBagBuilder` (Milestone 4). Milestone 2 does not need to implement it; the default `CreateVariableBag` override in Milestone 2 just materializes everything in-memory.

Tests to update or add: the existing `VariableEdgeTests`, `VariableGetSetTests`, `VariableObserveTests`, `VariableEndToEndTests`, `VariableBagTests`, `VariableWiringTests` under `Assets/Packages/com.scaffold.graphflow/Tests/` all run the same scenarios through the new shared types. The body changes are mechanical (namespace updates, `Set` instead of `Value =`, `Subscribe` instead of `Changed +=`). One new test asserts that a `GraphRunner` subclass overriding `CreateVariableBag` to return a custom bag implementation receives that bag as `runner.Variables`. Existing CardSandbox and other sample tests must remain green.

Validation: from the repository root run `.agents/scripts/validate-changes.cmd` and expect no errors. Run `.agents/scripts/run-editmode-tests.ps1` and observe all graphflow tests passing, including the new `CreateVariableBag` override test. Open the project in Unity and verify there are no `error CS` lines in the editor console. Acceptance: graphflow no longer contains its own variable interface or bag class; the seam is repositioned; all tests green.
...
For Milestone 2, the GraphFlow migration follows this order. First, delete the four legacy files in `Assets/Packages/com.scaffold.graphflow/Runtime/Variables/` (`IVariableBag.cs`, `VariableCell.cs`, `InMemoryVariableBag.cs`, `VariableDefault.cs`) along with their `.meta` siblings. Second, update every `using` directive across the runtime and tests to reference `Scaffold.Variables` instead of `Scaffold.GraphFlow` for these types. Third, edit `Assets/Packages/com.scaffold.graphflow/Runtime/Asset/RuntimeVariable.cs` to reference `Scaffold.Variables.VariableDefault`. Fourth, edit `Assets/Packages/com.scaffold.graphflow/Runtime/Controller/GraphRunner.cs`: remove `CreateParentBag()` and `SeedVariables()`; add `protected internal virtual IVariableBag CreateVariableBag(IEnumerable<RuntimeVariable> seed)` with the default in-memory implementation. Fifth, edit `Assets/Packages/com.scaffold.graphflow/Runtime/Builder/GraphBuilder.cs` to call `runner.Variables = runner.CreateVariableBag(asset.variables)` directly. Sixth, migrate `cell.Value = x` to `handle.Set(x)` in the `Set*VariableNode<T>` partials and `cell.Changed += ...` to `handle.Subscribe(...)` in the `Observe*VariableNode<T>` partials. Seventh, update the tests as described.
