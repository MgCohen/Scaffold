...
- [ ] Bring the working branch up to date with `origin/main` (merge or rebase) before any code work starts. The plan files are the only expected conflicts.
...
- [ ] Milestone 2 — Migrate GraphFlow. Delete `IVariableBag.cs`, `VariableCell.cs`, `InMemoryVariableBag.cs`. Edit `BlackboardVariable.cs` to extend shared `VariableDefault<T>`. Replace `runner.CreateParentBag()` with `runner.CreateVariableBag(seed)`; migrate `cell.Value = x` call sites to `handle.Set(x)` and `cell.Changed +=` to `handle.Subscribe`; migrate consumer samples (`VariableShowcase`, `StrikeWithVariables`) and `VariableFollowUpTests`. All existing GraphFlow tests must stay green.
