...
- `EntityInstanceTests` and **`AttributeBagTests`** (`Tests/Runtime/`) cover definition resolution, flyweight caching, three-bag reads, float modifier sum, **`ClearModifiers`** restoring base values, runtime base preserved after modifier removal, invalid removal, runtime attribute slots, structural events, silent effective-bag writes, `CreateOnGameObject`, and positive **`InstanceId.Id`**. **`EntityComponentEditor`** shows the definition **`AttributeBag`** read-only in the inspector; **`AttributeBagPropertyDrawer`** edits serialized entry lists.
