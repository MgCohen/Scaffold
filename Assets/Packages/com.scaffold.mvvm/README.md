...
| `BindingOptions` | Shared options for strict/lazy binding behavior and optional per-bind update timing. | Option flags/values; `UpdateTiming` null inherits bind-source default. | Consistent options payload for bind registration. | Invalid combinations are handled by bind consumers. |
| `BindingUpdateTiming` / `DeferredBindingCoroutineHost` | Deferred binding refresh (next frame / end of frame) via a lazy-created global coroutine host (DontDestroyOnLoad). | Policy from `RegisterBindingUpdatePolicy` and per-bind `BindingOptions`. | Batches deferred updates per `TreeBinding` flush; immediate mode still updates on every `UpdateBind`. | First deferred schedule creates the host `GameObject` if needed. |
...
5. Optional: call `RegisterBindingUpdatePolicy(BindingUpdateTiming)` once on the bind source (same pattern as converters) so all binds inherit deferred refresh; override a single bind with `BindingOptions` (`UpdateTiming` or `BindingOptions.StrictImmediate`). Deferred work is scheduled through `DeferredBindingCoroutineHost` (no injection).
...
- Added deferred binding updates: `BindingUpdateTiming`, `RegisterBindingUpdatePolicy`, per-bind `BindingOptions.UpdateTiming`, and `DeferredBindingCoroutineHost` (global coroutine pump).
