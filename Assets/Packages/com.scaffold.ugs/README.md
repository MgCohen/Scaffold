...
- Depends on: `com.scaffold.layeredscope` (`Scaffold.LayeredScope.IAsyncInitializable`), `Unity.Services.Core`, `Unity.Services.Authentication`, `VContainer`.
...
- Owns `Ugs` (`Scaffold.LayeredScope.IAsyncInitializable`) that ensures `UnityServices` is initialized and the player is signed in anonymously when no session exists.
- Owns `UgsInstaller` registering `Ugs` as singleton and as `Scaffold.LayeredScope.IAsyncInitializable`.
...
4. Ensure your host runs the LayeredScope init wave for each pushed layer (e.g. `ApplicationBootstrap` / `ApplicationHost`) so `Ugs` executes before modules that assume UGS is ready.
...
2. Let your startup host resolve all `Scaffold.LayeredScope.IAsyncInitializable` services after each layer’s container is built (same pattern as `LiveOpsService`, etc.).
...
- Allowed Dependencies:
  - `com.scaffold.layeredscope`, Unity Services Authentication/Core packages, VContainer.
...
- Change Checklist:
  ...
  - confirm async initializer ordering with your startup host (LayeredScope runs `IAsyncInitializable` instances registered in each layer).
- Known Tricky Areas:
