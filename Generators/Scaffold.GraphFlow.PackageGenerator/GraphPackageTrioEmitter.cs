...
namespace Scaffold.GraphFlow.PackageGenerator
{
    internal static class GraphPackageTrioEmitter
    {
        ...
        {
            // Per ExecPlan-v2 M2 milestone — initial OnGraphChanged rule set ships the two checks
            // tractable from registry metadata: duplicate-entry and unsupported-node-type. The richer
            // rules (required-input-unwired, type-mismatch, dangling flow edges, run-flow termination)
            // need richer port introspection and land in M3+ as the GT API surface gets explored.
            ...
            sb.AppendLine("                {");
            sb.AppendLine("                    logger.LogError($\"Unsupported node type {node.GetType().Name} — no registry entry. Re-baking will fail.\", node);");
            ...
            sb.AppendLine("                {");
            sb.AppendLine("                    logger.LogWarning($\"Duplicate entry node for '{kv.Key}'. The bake step keeps only the first occurrence.\", dup);");
            sb.AppendLine("                }");
            sb.AppendLine("            }");
            sb.AppendLine("        }");
            sb.AppendLine("    }");
