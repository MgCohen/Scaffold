...
| **com.scaffold.layeredscope** | Stacked VContainer `LifetimeScope` layers: `ApplicationBootstrap` / `ApplicationHost`, ordered install, `IAsyncInitializable` init waves, optional `ILayerPublisher` for cross-layer registration. | Subclass `ApplicationBootstrap`; return `IScopeLayer` instances from `GetInitialLayers()`. | `https://github.com/MgCohen/Scaffold.git?path=/Assets/Packages/com.scaffold.layeredscope#main` | [Assets/Packages/com.scaffold.layeredscope](Assets/Packages/com.scaffold.layeredscope) |
