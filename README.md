...
| **com.scaffold.appflow** | Stacked VContainer `LifetimeScope` layers: `AppFlowRoot` / `AppFlowHost`, ordered install, `IAsyncInitializable` init waves, `IAppFlowErrorHandler`, `IAppFlowProgress`, optional `ILayerPublisher` for cross-layer registration. | Subclass `AppFlowRoot`; return `IScopeLayer` instances from `GetInitialLayers()`. | `https://github.com/MgCohen/Scaffold.git?path=/Assets/Packages/com.scaffold.appflow#main` | [Assets/Packages/com.scaffold.appflow](Assets/Packages/com.scaffold.appflow) |
